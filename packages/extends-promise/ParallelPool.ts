...
type ParallelExector<T> = () => $PromiseMaybe<T>;
...
export class ParallelPool<T = any> {
  ...
  /**正在执行中的任务集合 */
  private _runningTasks: Promise<
    ParallelSuccessRunningReturn<T> | undefined
  >[] = [];
  ...
  get isFull() {
    return (
      this._runningTasks.length + this._waitingQuene.length >=
      this.maxParallelNum
    );
  }
  ...
  private async *_yieldResults(opts: YieldParallelOptions = {}) {
    ...
    do {
       ...
       */
      while (
        parallelTaskList.length < this.maxParallelNum &&
