...
import { FastQuene } from "@bfchain/util-extends-array";
...
export class MixAsyncIterator<
  T extends BFChainUtil.MixAsyncIterator.AG = BFChainUtil.MixAsyncIterator.AG,
> {
  private _allGenList = new FastQuene<BFChainUtil.MixAsyncIterator.Gen<T>>();
  join(
  ...
  private _join(gen: BFChainUtil.MixAsyncIterator.Gen<T>) {
    this._allGenList.push(gen);
    /// 否则保存到惰性列表中，需要时再使用
    if (gen.bench) {
      this._benchGen.push(gen as BFChainUtil.MixAsyncIterator.Gen<T, true>);
    }
    /// 如果是非惰性的，那么直接启动迭代
    else {
      this._handleGen(gen);
  ...
  remove(gen: BFChainUtil.MixAsyncIterator.Gen<T>) {
  ...
  private _enqueueGen(gen: BFChainUtil.MixAsyncIterator.Gen<T>) {
    // 是否有在等待中的
    ...
    /// 如果是非惰性元素，那么回到自由队列
      ...
      this._freeGen.push(gen as BFChainUtil.MixAsyncIterator.Gen<T, false>);
    }
  }
  ...
  private _runnings = new Map<
    ...
    BFChainUtil.MixAsyncIterator.Gen<T>
  >();
  private _handleGen(gen: BFChainUtil.MixAsyncIterator.Gen<T>) {
  ...
  /**候补队列 */
  private _benchGen = new FastQuene<BFChainUtil.MixAsyncIterator.Gen<T, true>>();
  /**自由队列 */
  private _freeGen = new FastQuene<BFChainUtil.MixAsyncIterator.Gen<T, false>>();
  private _genWaitter = new FastQuene<PromiseOut<BFChainUtil.MixAsyncIterator.Gen<T>>>();
  /**入列
   * 这个函数与_handleGen是紧密关联的，是_handleGen的一部分
   */
  private _enqueueResult(result: BFChainUtil.MixAsyncIterator.ResultWithFrom<T>) {
    /// 是否有在等待中的
    const waiter = this._resultWaiter.shift();
    if (waiter !== undefined) {
      /**
      ...
      result.from.bench
        ? this._benchResult.push(result as BFChainUtil.MixAsyncIterator.ResultWithFrom<T, true>)
        : this._freeResult.push(result as BFChainUtil.MixAsyncIterator.ResultWithFrom<T, false>);
  ...
  private _freeResult = new FastQuene<BFChainUtil.MixAsyncIterator.ResultWithFrom<T, false>>();
  private _benchResult = new FastQuene<BFChainUtil.MixAsyncIterator.ResultWithFrom<T, true>>();
  private _resultWaiter = new FastQuene<PromiseOut<BFChainUtil.MixAsyncIterator.Result<T>>>();
  // readonly onError = new Evt<MixAsyncIterator.Error<T>>();
  next() {
    if (this.isDone) {
      return DONE as Promise<BFChainUtil.MixAsyncIterator.Result<T>>;
    }
