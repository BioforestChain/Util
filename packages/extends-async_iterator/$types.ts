...
export namespace $MixAsyncIterator {
  ...
  export type Gen<T extends AG, B extends boolean = boolean> = {
    name: string;
  ...
  export type Result<T extends AG> = {
    step: IteratorResult<T[0], T[1]>;
    from?: Gen<T>;
  };
  export type ResultWithFrom<T extends AG, B extends boolean = boolean> = {
    step: IteratorResult<T[0], T[1]>;
    from: Gen<T, B>;
  };
  export type Error<T extends AG> = {
    from: Gen<T>;
    reason: unknown;
  };
}
