...
export function transformNamespaceToFilter(namespace: string) {
  ...
  return {
    ...
    source: namespace,
    regexp: new RegExp(
      "^" + namespace.substr(disable ? 1 : 0).replace(/\*/g, ".*?") + "$"
    ),
  } as $NamespaceFilter;
}
export function transformFilterToNamespace(filter: $NamespaceFilter) {
  return (
    (filter.type === "disable" ? "-" : "") + regexpToNamespace(filter.regexp)
  );
}
...
export abstract class DebugCreater<ColorType> {
   ...
   */
  public readonly namespaceFilters: $NamespaceFilter[] = [];
  /**
  ...
  private _resetInstancesEnabled() {
    for (const instance of this._createdDebugCache.values()) {
      instance.forceSetEnabled(
        this.isEnabled(instance.namespace, instance.level)
      );
  ...
  addNamespaceFilter(filter: string | $NamespaceFilter, pos?: number) {
  ...
  //#region Level
  private _levelFilter: $LevelFilter = (level) => true;
  public get levelFilter(): $LevelFilter {
    return this._levelFilter;
  }
  public set levelFilter(value: $LevelFilter) {
   ...
   * @api public
   */
  isEnabled(name: string, level: $DebugLevel) {
...
export class Debug<ColorType> {
  private _config: $DebuggerConfig<ColorType>;
  get useColors() {
  ...
  public readonly scopePrinterMap = EasyMap.from<$Scope<ColorType>, $Pinter>({
    creater: (scope) => {
  ...
  private _printerBuilder(scope: $Scope<ColorType>) {
    const { _creater } = this;
    const isEnabled = () => scope.enabled && this.enabled;
    const print: $Pinter = ((...args: unknown[]) => {
      // Disabled?
      if (!isEnabled()) {
        return;
      }
      ...
      let format =
        typeof args[0] !== "string" ? "%O" : (args.shift() as string);
      ...
      const logArgs = [format, ...args] as $LogArgs;
