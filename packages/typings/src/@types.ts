...
declare namespace BFChainUtil {
  ...
  /**函数的第三个参数类型 */
  type ThirdArgument<T> = T extends (arg1: any, arg2: any, arg3: infer U, ...args: any[]) => any
    ? U
    : any;

  /**函数的所有参数类型 */
  type AllArgument<T> = T extends (...args: infer U) => any ? U : any;
  type GeneratorYieldType<G extends Generator> = G extends Generator<infer Y> ? Y : never;
  type GeneratorReturnType<G extends Generator> = G extends Generator<any, infer Y> ? Y : never;
  ...
  /**往一个数组前面面插入一个元素 */
  type ArrayUnshift<T extends any[], A> = ((a: A, ...b: T) => void) extends (...a: infer R) => void
    ? R
    : never;
  type ArrayShift<T extends any[]> = ((...a: T) => void) extends (a: any, ...b: infer R) => void
    ? R
    : never;
