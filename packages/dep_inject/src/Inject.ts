...
import { getInjectionToken } from "./common";
...
export function Inject(
  inject_module?: BFChainUtil.MODULE_ID_TYPE | BFChainUtil.Constructor<object>,
  conf?: BFChainUtil.InjectModuleConfig,
) {
  return (target: any, prop_name: string, index?: number | PropertyDescriptor) => {
    if (typeof index === "number") {
      const module_name =
        typeof inject_module === "function" ? getInjectionToken(inject_module) : inject_module;
      if (module_name !== undefined) {
        const modules: BFChainUtil.FactoryCustomInjectModule =
...
export function DirtyInjectProp(
  inject_module_getter: () => BFChainUtil.MODULE_ID_TYPE | BFChainUtil.Constructor<object>,
  conf?: BFChainUtil.InjectModuleConfig,
) {
...
function getInjectPropsMetadataMap(target: any, matedataKey: string) {
  let map: BFChainUtil.AutoResolveMatedata | undefined = Reflect.getMetadata(matedataKey, target);
  if (!map) {
    map = new Map();
