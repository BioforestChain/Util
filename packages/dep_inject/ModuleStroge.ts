...
import { InjectionToken, getInjectionGroups } from "./common.ts";
...
export class ModuleStroge implements Map<$MODULE_ID_TYPE, any> {
  clear(): void {
  ...
  private _stroge: Map<$MODULE_ID_TYPE, any>;
  constructor(
    entries?: ReadonlyArray<[$MODULE_ID_TYPE, any]> | null,
  ...
  installMask(mask: ModuleStroge): ModuleStroge {
    const maskGet = (key: $MODULE_ID_TYPE) => {
    ...
    const maskHas = (key: $MODULE_ID_TYPE) => {
      /**
       * 优先从面具集群中寻找
       */
   ...
   * 组收集器
   */
  private _groupCollection?: EasyMap<$MODULE_ID_TYPE, Set<any>>;
   ...
   * @param instance
   */
  groupInsert(groupName: $MODULE_ID_TYPE, instance: unknown) {
  ...
  groupGet<T = unknown>(
    ...CtorRootList: Array<
      InjectionToken<T> | $Constructor<T> | $MODULE_ID_TYPE | $MODULE_ID_TYPE[]
    >
  ) {
   ...
   * @param groups
   */
  private groupsGet_<T>(groups: $MODULE_ID_TYPE[]) {
    groups = groups.slice();
    let result: Set<T> | undefined;
   ...
   * @param groupName
   */
  private groupGet_<T>(groupName: $MODULE_ID_TYPE) {
    const result: T[] = [];
