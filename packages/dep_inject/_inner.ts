...
import { $FactoryCustomInjectModule } from "./$types.ts";
import type { ModuleStroge } from "./ModuleStroge.ts";
 ...
 * 对象与其ModelStroge的映射
 */
export const INSTANCE_MODULES_WM = new WeakMap<any, ModuleStroge>();
/**
 ...
 */
export const beforeCreateInstanceModuleStrogeStack: ModuleStroge[] = [];
export function getInstanceModules(ins: any) {
  return (
    INSTANCE_MODULES_WM.get(ins) ||
    beforeCreateInstanceModuleStrogeStack[
      beforeCreateInstanceModuleStrogeStack.length - 1
    ]
  );
}
