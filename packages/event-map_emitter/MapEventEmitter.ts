...
> implements $EventEmitterMix<EM, EM2>
{
  /**导出类型 */
  TYPE!: EMX;

  private _e: $EventHanldersMap<EMX> = Object.create(null);
  /**
   * 监听事件
   * 在这里重复的事件不会被重复监听
  ...
  private _on<EN extends keyof this["TYPE"]>(
    eventname: EN,
    handler: $MutArgEventHandler<EMX[EN]>,
    taskname?: string,
    once?: boolean
  ) {
  ...
  private get _emitErrorHandlerSet() {
    ...
    return new Set<$InnerErrorHandler<EMX>>();
  }
  /**
   * 自定义函数执行异常处理器
   * @param errorHandler
   */
  onError(errorHandler: $InnerErrorHandler<EMX>, taskname?: string) {
   ...
   * @param errorHandler
   */
  offError(errorHandler?: $InnerErrorHandler<EMX>) {
    if (!this._hasEmitErrorHandlerSet) {
      return false;
    }
