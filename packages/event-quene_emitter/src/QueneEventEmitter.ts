...
import { isAsyncGeneratorInstance, isGeneratorInstance } from "@bfchain/util-extends-iterator-is";

/**一个极简的事件管理模块 */
export class QueneEventEmitter<EM extends BFChainUtil.EventInOutMap>
  implements BFChainUtil.QueneEventEmitter<EM>
{
  // IN_K = K | "*",
  TYPE!: EM;
  private _e: BFChainUtil.EventInOutHanldersMap<EM> = Object.create(null); // = new Map<EventName, Set<BFChainUtil.InOutEventHandler>>();
  /**
   * 增加监听事件
   * @param eventname 事件名
   * @param handler 处理函数
  ...
  on<K extends keyof EM>(
    eventname: K,
    handler: BFChainUtil.InOutEventHandler<EM[K]>,
  ...
  once<K extends keyof EM>(
    eventname: K,
    handler: BFChainUtil.InOutEventHandler<EM[K]>,
    opts?: Omit<BFChainUtil.EventOptions, "once">,
  ) {
    return this.on(
      eventname,
      handler,
  ...
  has<K extends keyof EM>(eventname: K, handler?: BFChainUtil.InOutEventHandler<EM[K]>) {
  ...
  off<K extends keyof EM>(eventname: K, handler: BFChainUtil.InOutEventHandler<EM[K]>) {
    const handlerMap = this._e[eventname];
    let deleted = false;
    /// 移除事件监听，如果事件存储器空了，那么删除存储器
    if (handlerMap && (deleted = handlerMap.delete(handler)) && handlerMap.size === 0) {
      ...
      const openAg = (
        ...
        if (typeof res === "object") {
          if (isAsyncGeneratorInstance(res)) {
            return (async () => {
              try {
                const ag = (res as AsyncGenerator<unknown, EM[K]["out"]>)[Symbol.asyncIterator]();
                do {
            ...
            const sg = (res as Generator<unknown, EM[K]["out"]>)[Symbol.iterator]();
            try {
              do {
                const item = sg.next();
                if (item.done) {
  ...
  async *emitToAsyncGenerator<K extends keyof EM>(eventname: K, data: EM[K]["in"]) {
    const handlerMap = this._e[eventname];
    if (handlerMap) {
      if (isDev) {
        if (handlerMap.size) {
  ...
  protected _emitErrorHanlder<K extends keyof EM>(
    ...
    if (this._hasEmitErrorHandlerSet) {
      ...
      if (isDev) {
        console.error(
          `QueneEventEmitter '${this.constructor.name}' emit '${eventname.toString()}' fail:`,
          err,
        );
      }
      throw err;
  ...
  private get _emitErrorHandlerSet() {
    ...
    return new Set<BFChainUtil.InnerErrorInOutHandler<EM>>();
  }
  /**
   * 自定义函数执行异常处理器
   * @param errorHandler
   */
  onError(errorHandler: BFChainUtil.InnerErrorInOutHandler<EM>, taskname?: string) {
   ...
   * @param errorHandler
   */
  offError(errorHandler?: BFChainUtil.InnerErrorInOutHandler<EM>) {
    if (!this._hasEmitErrorHandlerSet) {
      return false;
    }
...
// export function syncQueneEventEmitter<EM extends BFChainUtil.EventInOutMap>() {
//   return new QueneEventEmitter() as BFChainUtil.QueneEventEmitter<EM>;
// }
...
// (async () => {
//   const qee = new QueneEventEmitter<{ qaq: BFChainUtil.EventInOut<Uint8Array, Uint8Array> }>();
//   {
