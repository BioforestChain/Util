import { $Lock } from "./$types.ts";

let uuid_accer = 0;
/* 基于callback风格的锁，兼容Promise，性能比Promise好 */
export class AtomLock<
  K extends $Lock.AtomLockKey = $Lock.TransferableAtomLockKey
> {
...
export function isAtomLockKey(key: unknown): key is $Lock.AtomLockKey {
  ...
  appendLocksWithDetail(ids: Iterable<K>) {
    ...
    for (const id of ids) {
      ...
      if (i === 256) {
        // > 255
        console.warn(
          "Too many locks at once, please divide the task reasonably."
        );
        is_over_uint8 = true;
      }
      if (i === 65536) {
