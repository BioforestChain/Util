import { $SafeThendCache } from "./$types.ts";

const SAFE_THENED_WM = new WeakMap<PromiseLike<any>, $SafeThendCache<any>>();
...
export function safePromiseThen<T, TResult1 = T, TResult2 = never>(
  ...
  if (!thened) {
    const cbs: $SafeThendCache<T>["cbs"] = {
      resolves: new Set(),
      rejects: new Set(),
    };
        ...
        for (const rejectFun of cbs.rejects) {
          queueMicrotask(async () => {
            try {
              ...
              console.error(
                "Unhandled promise rejection when running onrejected",
                rejectFun,
                err
              );
            }
    ...
    const _thened: $SafeThendCache<T> = {
      cbs,
      promise: toPromise,
      // isFinished: false,
    };
  ...
  return { thened: thened as $SafeThendCache<T>, toPromise };
}
...
export function safePromiseOffThen<T, TResult1 = T, TResult2 = never>(
  ...
  return thened as $SafeThendCache<T>;
}
