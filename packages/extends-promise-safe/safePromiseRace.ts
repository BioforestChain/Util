import { safePromiseThen } from "./safePromiseThen.ts";
import { PromiseOut } from "../extends-promise-out/index.ts";
import { $PromiseType } from "../typings/$types.ts";
import { $PromiseRaceOptions } from "./$types.ts";

const enum RANK_TYPE {
  Rejected,
  Resolved,
}

type $RankItem<TS> =
  | { type: RANK_TYPE.Resolved; value: TS }
  | { type: RANK_TYPE.Rejected; value?: unknown };
type $Thened<TS> = {
  cbs: {
 ...
 * Promise 竞争器
 */
export class PromiseRace<T, TS = $PromiseType<T>> {
  constructor(options?: Partial<$PromiseRaceOptions>) {
    this.options = PromiseRace.mergeOptions(options);
  }
  /**配置 */
  public options: $PromiseRaceOptions;
  /**合并选项 */
  static mergeOptions(options?: Partial<$PromiseRaceOptions>) {
    const defaultOptions: $PromiseRaceOptions = {
  ...
  private resolve(value: TS) {
    if (this.quota) {
      ...
      this.rankList.push({ type: RANK_TYPE.Resolved, value });
    }
  }
  private reject(reason?: unknown) {
    if (this.quota) {
      ...
      this.rankList.push({ type: RANK_TYPE.Rejected, value: reason });
    }
  }
  /**增加竞争者 */
  addRacer(promise: PromiseLike<TS>) {
    ...
    if (!thended) {
      ...
      const { thened, toPromise } = safePromiseThen(
        promise,
        onfulfilled,
        onrejected
      );
  ...
  race(outter?: PromiseOut<TS>) {
    ...
    if (inRank.type === RANK_TYPE.Resolved) {
      return inRank.value;
    } else {
      throw inRank.value;
    }
