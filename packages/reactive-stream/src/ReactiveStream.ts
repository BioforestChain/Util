...
export class ReactiveStream<T> /* implements AsyncIterableIterator<T>  */ {
  ...
  async *map<R>(map: BFChainUtil.ReactiveStream.Map<T, R>) {
    let index = 0;
    loop: for await (const item of this._src) {
      const event: BFChainUtil.ReactiveStream.Map.Event<T> = {
        data: item,
        index: index++,
        statement: undefined,
        target: this,
...
//   ): R | BFChainUtil.PromiseMaybe<$ForController>;
// }
...
// ) => O | BFChainUtil.PromiseMaybe<$ForController>;
...
// //   filter: (item: T, index: number, ctx: ReactiveStream<T>) => BFChainUtil.PromiseMaybe<boolean>,
// // ): $PipeHanlder<T, R> {
...
// //     | ((item: T, index: number) => BFChainUtil.PromiseMaybe<boolean>)
