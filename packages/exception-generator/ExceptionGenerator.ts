import { CustomError } from "../extends-error/index.ts";
import { ErrorCode } from "../exception-error-code/index.ts";
import { cacheObjectGetter } from "../extends-object/index.ts";
import { $Exception, $ExceptionConstructor } from "./$types.ts";
...
export abstract class Exception extends CustomError implements $Exception {
  ...
  constructor(
    ...
    const ExceptionCtor = this.constructor as $ExceptionConstructor;
  ...
  static is<E extends $Exception>(
    this: $ExceptionConstructor<E>,
    err: any
  ): err is E {
...
//#endregion
if (
  typeof (Error as unknown as $ErrorConstructor).stackTraceLimit === "number"
) {
  (Error as unknown as $ErrorConstructor).stackTraceLimit += 3;
}
...
export function ExceptionGenerator(
  ...
  const getException = <E extends $Exception, C extends abstract new () => E>(
    Con: C
  ) => {
    const ExtCon = Con as unknown as $ExceptionConstructor<E>;
