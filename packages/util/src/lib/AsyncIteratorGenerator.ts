import { QueneEventEmitter } from "@bfchain/util-event";
...
export class AsyncIteratorGenerator<T> extends QueneEventEmitter<{
  requestAll: BFChainUtil.EventInOut<void>;
  requestItem: BFChainUtil.EventInOut</* index */ number, void>;
  push: BFChainUtil.EventInOut<{ item: T; index: number }, void>;
  pushCalc: BFChainUtil.EventInOut<{ calc: () => T; index: number }, void>;
  done: BFChainUtil.EventInOut<void>;
  error: BFChainUtil.EventInOut<unknown>;
}> {
  readonly list = [] as T[];
  /**下一个坐标的最小值 */
  min_next_index = 0;
  ...
  @bindThis
  then(...args: BFChainUtil.AllArgument<Promise<T[]>["then"]>) {
    return this._initTask().then(...args);
  }
  @bindThis
  catch(...args: BFChainUtil.AllArgument<Promise<T[]>["catch"]>) {
        ...
        if (need_index in list) {
          ...
          if (!self.is_done) {
            res = await new Promise(async (resolve, reject) => {
              ...
              const wait_error: BFChainUtil.EventHandler<unknown> = (err, next) => {
                off_bind();
                reject(err);
                return next();
              };
              const wait_done: BFChainUtil.EventHandler<void> = (_, next) => {
                off_bind();
                resolve({
                  done: true,
                  value,
              ...
              const wait_next: BFChainUtil.EventHandler<{
                item: T;
                index: number;
              }> = ({ item, index }, next) => {
                if (need_index === index) {
              ...
              const wait_next_calc: BFChainUtil.EventHandler<{
                calc: () => T;
                index: number;
              }> = ({ calc, index }, next) => {
                if (need_index === index) {
              ...
              ///
              try {
                await self.emit("requestItem", (self._requestProcess = need_index));
              } catch (err) {
                self.emit("error", err);
              }
            });
...
type FilterMapDefault = { filter?: false; map?: unknown; maps?: unknown; multi?: false };
...
export const AsyncIteratorGeneratorTransfer = <F, T>(
  ...
  const getPoAtList = (poList: NonNullable<typeof reqNextPoList>, index: number) => {
    let po = poList[index];
    if (po === undefined) {
