import { Inject, Injectable } from "../dep_inject/index.ts";
import { UtilExceptionGenerator } from "../exception/index.ts";
import { sleep } from "../extends-promise/index.ts";
import { PlatformHelper } from "./platformHelper.ts";
...
export class TimeHelper {
  ...
  now(forceRounding = false) {
    const pnow =
      this.platformHelper.performance.now() +
    ...
    if (diffMs !== 0) {
      if (oldDiff === undefined) {
        ...
        const customNow = (Date.now = () =>
          sourceNow() + Reflect.get(Date, NOW_DIFF_SYMBOL));
  ...
  formatYMDHMS(
    ...
    const ps = (num: number, maxLength = 2) =>
