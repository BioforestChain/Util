...
import { MapEventEmitter } from "@bfchain/util-event-map-emitter";

export class MapEventEmitterPro<EM = {}, EM2 = never> extends MapEventEmitter<EM, EM2> {
  /**
   * 触发事件
   * @param eventname
   * @param args
  ...
  protected _emit<K extends keyof this["TYPE"]>(
    eventname: K,
    args: BFChainUtil.MutArg<this["TYPE"][K]>,
  ) {
    /**
     * 处理前置的任务
     */
      ...
      for (const [commonHanlder, opts] of this._commonEmitHandlerMap) {
        try {
          if (isDev) {
            ...
            console.log(
              ...eventDebugStyle.head("RUN [%s]", eventDebugStyle.MIDNIGHTBLUE_BOLD_UNDERLINE),
              taskname,
            );
          }
  ...
  private get _commonEmitHandlerMap() {
    ...
    return new Map<BFChainUtil.InnerAnyHandler<this["TYPE"]>, BFChainUtil.EventOptions>();
  }
  /**
   * 监听所有事件
   * @param commonHanlder
   ...
   */
  onEmit(
    commonHanlder: BFChainUtil.InnerAnyHandler<this["TYPE"]>,
   ...
   * @param commonHanlder
   */
  offEmit(commonHanlder: BFChainUtil.InnerAnyHandler<this["TYPE"]>) {
    if (!this._hasCommonEmitHandlerMap) {
      return false;
    }
  ...
  private get _afterTasks() {
    ...
    return Object.create(null) as BFChainUtil.EventWaitterMap<this["TYPE"]>;
  }
  /**
   * 与once不同，返回promise
