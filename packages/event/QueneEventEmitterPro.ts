...
import { QueneEventEmitter } from "../event-quene_emitter/index.ts";
import {
  $EventInOutMap,
  $InnerAnyInOutHandler,
} from "../event-quene_emitter/$types.ts";
import { $QueneEventEmitterPro } from "./$types.ts";

/**一个极简的事件管理模块 */
export class QueneEventEmitterPro<EM extends $EventInOutMap>
  extends QueneEventEmitter<EM>
  implements $QueneEventEmitterPro<EM>
{
  /**触发事件监听 */
  ...
  private get _commonEmitHandlerMap() {
    ...
    return new Map<$InnerAnyInOutHandler<EM>, $EventOptions>();
  }
  /**
   * 监听所有事件
   * @param commonHanlder
   * @param taskname
   */
  onEmit(commonHanlder: $InnerAnyInOutHandler<EM>, opts: $EventOptions = {}) {
   ...
   * @param commonHanlder
   */
  offEmit(commonHanlder: $InnerAnyInOutHandler<EM>) {
    if (!this._hasCommonEmitHandlerMap) {
      return false;
    }
...
export function syncQueneEventEmitterPro<EM extends $EventInOutMap>() {
  return new QueneEventEmitterPro() as $QueneEventEmitterPro<EM>;
}
