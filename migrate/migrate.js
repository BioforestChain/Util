...
const init = async () => {
  const { fileDirs, filesArrs } = await getWorkspaceRoot(workspaceRoot)
...
const nodeMigrateFactory = (nodeFiles) => {
  createWriteStream(opinionFile,nodeFiles ,'以下文件不符合pkgm的*#node.ts风格' ,{ 'flags': 'a'});
}

const declareMigrateFactory = (declareFiles) => {
  createWriteStream(opinionFile,declareFiles ,'@types.ts 这种文件，只用来declare，不可以出现import <spe>' ,{ 'flags': 'a'});
}
